...
@(init)
init :: proc() {
	INDEX = os.lookup_env("INDEX") or_else "http://localhost:6969"
}
...
main :: proc() {
	...
	rate_limited := http.rate_limit(
		&rate_limit_data,
		&sessioned,
		&{window = time.Second * 30, max = 30, on_limit = http.rate_limit_message(&limited_msg)},
	)
	...
	err := http.listen_and_serve(&server, logged, net.Endpoint{address = net.IP4_Any, port = 6969})
...
handler_create_todo :: proc(req: ^http.Request, res: ^http.Response) {
	...
	http.body(
		...
		-1,
		res,
		proc(res: rawptr, raw_body: http.Body, err: http.Body_Error) {
...
handler_toggle :: proc(req: ^http.Request, res: ^http.Response) {
	...
	for todo in session.list {
		switch {
		case todo.completed && all_completed:
			session.completed -= 1
		case !todo.completed && !all_completed:
			session.completed += 1
		}
	...
	respond_list(
		...
		false,
		session,
		page_from_path(http.headers_get_unsafe(req.headers, "hx-current-url")),
	)
}
