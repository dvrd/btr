...
url_parse :: proc(raw: string, allocator := context.allocator) -> URL {
	url: URL
	url.raw = raw
	url.scheme, url.host, url.path, url.queries, _ = net.split_url(raw, allocator)
	return url
}

url_string :: proc(url: URL, allocator := context.allocator) -> string {
	return net.join_url(url.scheme, url.host, url.path, url.queries, "", allocator = allocator)
}
