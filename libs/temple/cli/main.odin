...
main :: proc() {
	calls := collect_compile_calls("src")
	free_all(context.temp_allocator)

	transpile_calls("libs/temple", calls[:])
}
				...
				visit = proc(visitor: ^ast.Visitor, node: ^ast.Node) -> ^ast.Visitor {
					...
					case ^ast.Call_Expr:
						...
						if selector.field.name != "compiled" &&
						   selector.field.name != "compiled_inline" {
							return nil
						}
						...
						case "compiled":
							...
							cp.fullpath = filepath.join(
								{s.pkg.fullpath, unqouted_path},
								s.allocator,
							)

						case "compiled_inline":
							cp: Call_Inline
							cp.template = strings.clone(
								strings.trim(path.tok.text, "\"`"),
								s.allocator,
							)
							c.type = cp
						}
